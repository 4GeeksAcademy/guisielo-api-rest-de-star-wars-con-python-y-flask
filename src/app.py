...
def add_character():
    ...
    try:
        ...
        raise APIException(f"Failed to create character",
...
def get_all_favorites():
    ...
    if not user_id:
        raise APIException("Missing user_id", status_code=400)
...
def add_favorite_planet(planet_id):
    ...
    if existing:
        raise APIException("Already in favorites", status_code=400)
    ...
    try:
        ...
        raise APIException(
            f"Failed to add planet to favorites", status_code=500) from e
...
def add_favorite_character(character_id):
    ...
    try:
        ...
        raise APIException(
            f"Failed to add character to favorites", status_code=500) from e
...
def delete_favorite_planet(planet_id):
    ...
    searched_favorite_planet = Favorite.query.filter_by(
        planet_id=planet_id, user_id=user_id).first()

    if not searched_favorite_planet:
        raise APIException(
            f"Favorite planet from user is not found", status_code=404)
...
def delete_favorite_character(character_id):
    ...
    searched_favorite_character = Favorite.query.filter_by(
        character_id=character_id, user_id=user_id).first()

    if not searched_favorite_character:
        raise APIException(f"Favorite character from user {user_id} not found", status_code=404)
